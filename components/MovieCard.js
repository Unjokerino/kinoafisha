...
export default function MovieCard(props) {
  ...
  return (
    <TouchableOpacity
      onPress={() => {
        let navigation = props.navigation.route ? props.navigation.route.navigation : props.navigation
      ...
      <View style={{ flexDirection: "row" }}>
        <Image
          ...
          source={{
            uri:
              props.poster !== ""
                ? props.poster ? props.poster : props.img_sobitiya
        ...
        <View style={{ width: 200, marginLeft: 20 }}>
          ...
          <View style={{ flexDirection: "row", marginBottom: 10,marginTop:5, }}>
            {props.soonOnScreens ? <TouchableOpacity style={styles.tag}>
              <Text style={styles.caption}>В кино с {moment(props.seanses[0].date).locale("ru", localization).format("D MMMM")}</Text>
            </TouchableOpacity> : <View style={{ flexDirection: "row"}}>
            {props.seanses && props.seanses.map((seans, index) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  style={styles.tag}
                >
                  <Text style={styles.caption}>{seans.date.split(" ")[1]}</Text>
                </TouchableOpacity>
              );
            })}
            </View>}
